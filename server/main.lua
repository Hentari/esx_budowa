...
AddEventHandler('esx_gardener:GiveItem', function()
	...
	if Quantity >= 20 then
		...
		TriggerClientEvent('esx:showNotification', _source, '~g~Praca wykonana otrzymałeś raport budowlany!')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~Nie masz przy sobie więcej raportów budowlanych')
				PlayersVente[_source] = false
			else
				local amount = Config.Zones.Vente.ItemRemove
				local item = Config.Zones.Vente.ItemRequires
				xPlayer.removeInventoryItem(item, amount)
				xPlayer.addMoney(math.random(20,40))
				TriggerClientEvent('esx:showNotification', _source, '~g~Oddałeś raporty więc otrzymujesz wypłatę!')
				Vente(_source)
			end
...
AddEventHandler('esx_gardener:startVente', function()
	...
	if PlayersVente[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~Odszedłeś od kierownika!')
		PlayersVente[_source] = false
	else
		PlayersVente[_source] = true
		TriggerClientEvent('esx:showNotification', _source, '~g~Oddawanie raportów budowlanych...')
		Vente(_source)
	end
end)
